...
import SmallTable from "../table-widgets/small-table/SmallTable.jsx";
...
const DynamicDataTable = (props) => {
  ...
  const [allColumns, setAllColumns] = useState(
    cachedCols?.length ? cachedCols : [...columnsModified]
  );
  ...
  const getWidthOfFixedColumns = useCallback(() => {
  ...
  useEffect(() => {
    if (columnCache && !cachedCols?.length) {
      localStorage.setItem(
        tableId,
        JSON.stringify([...fixedColumns, ...resizeColumns])
      );
      ...
      if (column.sortable) {
        if (column.type === "number") {
          result = result.sort((a, b) => {
            return column.sortingOrder === "asc"
              ? a[column.selector] - b[column.selector]
              : b[column.selector] - a[column.selector];
          });
        } else if (column.type === "date") {
          result = result.sort((a, b) => {
            return column.sortingOrder === "asc"
              ? new Date(a[column.selector]) - new Date(b[column.selector])
              : new Date(b[column.selector]) - new Date(a[column.selector]);
          });
        } else {
          result = result.sort((a, b) => {
            return column.sortingOrder === "asc"
              ? a[column.selector].localeCompare(b[column.selector])
              : b[column.selector].localeCompare(a[column.selector]);
          });
        }
  ...
  const getLeftDistanceOfFixedColumn = useMemo(() => {
    return (n = 0) => {
      if (n === 0) {
        return 0;
      } else if (n === 1) {
  ...
  const handleSelectAllRows = useCallback((e) => {
    const selectedRows = allData.map((d) => ({
      ...d,
  ...
  const dataSlice =
    data.length > rowPerPage
      ? [...columnsData].slice(indexOfFirstData, indexOfLastData)
      : [...columnsData];
    ...
    <>
      ...
      >
        {!isSmallScreen ? (
          <ResizableTable
            fixed={
              expandableRows && selectableRows
                ? fixedColumns?.length + 2
                : expandableRows || selectableRows
          ...
          >
                ...
                {expandableRows ? (
                  <th
                    style={{
                      width: "40px",
                      left: selectableRows
                        ? getLeftDistanceOfFixedColumn(0)
                        : 0,
                      position: "sticky",
                      zIndex: 1,
                    }}
            ...
            <tbody>
              ...
              <tr hidden={!filter}>
                {expandableRows ? (
                  <td className="fixed-cell" style={{ left: 0 }}></td>
                ) : null}
                  ...
                  <td
                    ...
                    style={{
                      ...
                      left:
                        c.isFixed &&
                        getLeftDistanceOfFixedColumn(
                          expandableRows ? indx + 1 : indx
                        ),
                    }}
                    className={`${c.isFixed ? "fixed-cell" : ""}`}
                  >
                    {filterArray.find((f) => f[c?.selector])?.[c.selector] ??
                      null}
                  </td>
                ))}
              </tr>
              ...
              {progressPending ? (
                ...
                <tr>
                  <td
                    colSpan={
                      expandableRows ? columns.length + 1 : columns.length
                    }
                    style={{
                      textAlign: "center",
                      paddingTop: "40px",
