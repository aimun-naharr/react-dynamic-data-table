...
const DynamicResizableTable = ( { className, mainClass, responsive, bordered, size, tableId, children, fixed = 0, ...props } ) => {
    ...
    return (
        <>
            <table
                ...
                // bordered={bordered}
                size={size}
                className={classnames( `${mainClass}`, `${className} resize-and-fixed-table` )}
                {...props}
            >
                {children}
            </table>
