...
export default function SmallTable(props) {
  ...
  const handleSelectAllRows = (e) => {
    const selectedRows = columnsData.map((d) => ({
      ...d,
      ...
      <tbody>
        <tr hidden={!filter}>
          ...
          {mergedColumns?.map((c, indx) => (
            <td
              ...
              style={{
                padding: "2px",
                width: c.maxWidth ? `${c.maxWidth}px` : "",
              }}
            >
        ...
        {progressPending ? (
          ...
          <tr>
            <td
              colSpan={
                expandableRows ? mergedColumns.length + 1 : mergedColumns.length
              }
              style={{
                textAlign: "center",
                paddingTop: "40px",
...
const TR = (props) => {
  ...
  return (
    <>
      ...
      {expand ? (
        <tr key={`${row.rowId}`}>
          <td colSpan={mergedColumns.length + 1}>
            <div className="expandable-component">
              {ExpandedComponent ? (
                <ExpandedComponent data={row} />
              ) : (
                <p>Add a custom component in ExpandedComponent prop</p>
              )}
            </div>
          </td>
        </tr>
